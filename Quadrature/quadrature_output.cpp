...
void QuadratureOutput::initialize(double initial_angle_axis1, double initial_angle_axis2)
{
    ...
    if (axis1.starting_angle == 0|| axis1.starting_angle==360)
        axis1.index = 1;
    else
        axis1.index = 0;
    ...
    if (num_axes == 2)
    {
        ...
        axis2.channel_b = 0;
        // since the channels are low, index can start high IF AND ONLY IF the starting angle is 0/360
        if (axis2.starting_angle == 0||axis2.starting_angle==360)
            axis2.index = 1;
        else
            axis2.index = 0;
    }
...
void QuadratureOutput::update(double current_angle_axis1, double current_angle_axis2)
{
    ...
    {
        std::cout<<"One or more axes are not calibrated. Please calibrate and try again.";
        return;
    }
...
{
    if (axis == 0)
        return axis1.channel_a;
    if (axis == 1)
        return axis2.channel_a;
    return 0;
}
...
{
    if (axis == 0)
        return axis1.channel_b;
    if (axis == 1)
        return axis2.channel_b;
    return 0;
}
...
{
    if (axis == 0)
        return axis1.position_count;
    if (axis == 1)
        return axis2.position_count;
    return 0;
}
...
{
    if (axis == 0)
        return axis1.calibrated;
    if (axis == 1)
        return axis2.calibrated;
    return false;
}
